...
/***------------------  BEGIN AUGEMENTED METHOD ----------------***/
   @Override
   public boolean isEdge(E fromKey, E toKey) // come back for comments
   {
	      //implement this function
		   if (isEmpty()) // check for empty graph
		   {
			   return false;
		   }
		   
		   Vertex fromVertex = first; //source
		   Vertex toVertex = first; //destination
		   
		    while (fromVertex != null && cmp.compare(fromKey,  fromVertex.data) != 0)  //check and give data to source
		    { // locate source vertex
		        fromVertex = fromVertex.pNextVertex;
		    }

		    
		    while (toVertex != null && cmp.compare(toKey,  toVertex.data) != 0) //check and give data to destination
		    { // locate destination vertex
		        toVertex = toVertex.pNextVertex;
		    }
			   
		    if (fromVertex == null || toVertex == null) // check if vertices exist
		    {
		        return false;
		    }

		    Edge edgeWalk = fromVertex.pEdge; // check for edge

		    while (edgeWalk != null) //is edge exists
		    {
	
		        if (edgeWalk.destination == toVertex) //and the is = to destination
		        {
		            return true; // edge located
		        }
		        edgeWalk = edgeWalk.pNextEdge; //move to next edge to continue testing / neighbors
		        		    }
		return false; //unable to locate edge
	   }
   ...
   long edgeCount = 0; // keep track of edges
   Vertex walkPtr = first; //counter to walk through

   while (walkPtr != null)   //until run out of vertices
   {
	   Edge edgeWalk = walkPtr.pEdge; //set edges data
	   
	   while (edgeWalk != null) //and until you run out of edges w/ data
	   {
		   edgeCount++; //add to the count of edges
		   edgeWalk = edgeWalk.pNextEdge; // move to / check 
	   }
	   walkPtr = walkPtr.pNextVertex; // move counter to next edge/ neighbors
   }
